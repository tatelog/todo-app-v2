...
const defaultData = {
  ...
  tags: [
    { id: 'tag-1', name: '重要', createdAt: new Date().toISOString() },
    { id: 'tag-2', name: '会議', createdAt: new Date().toISOString() }
  ]
};

const readData = () => {
  if (!fs.existsSync(DATA_FILE)) fs.writeFileSync(DATA_FILE, JSON.stringify(defaultData));
...
module.exports = (req, res) => {
  ...
  if (req.method === 'GET') {
    return res.json(id ? data.todos.find(t => t.id === id) : data.todos);
  }
  if (req.method === 'POST') {
    const todo = {
